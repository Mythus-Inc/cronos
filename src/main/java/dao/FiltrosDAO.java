...
public class FiltrosDAO implements Serializable{
	...
	public List certificadosAlunosImprimir(Long idAluno, int sit) {
		Query query = null;
			...
			query = manager.createQuery(
					"from Certificado a where a.status is true and a.alunoTurma = " + idAluno + " and a.situacao = " + sit+" order by atividadeTurma.atividade.grupo.abreviacao asc");
	...
	public List certificadosAlunos(Long idAluno, int sit) {
		Query query = null;
			...
			query = manager.createQuery(
					"from Certificado a where a.status is true and a.aluno = " + idAluno + " and a.situacao = " + sit+" order by atividadeTurma.atividade.grupo.abreviacao asc");
			return query.getResultList();
	
	}
	
	public List certificadosAlunosCodigoValidacao(String sit) {
		Query query = null;
	
			query = manager.createQuery(
					"from Certificado a where a.status is true and a.codigoAutenticacaoSistemaCertificados='" + sit+"' and (a.situacao=0 or a.situacao=1 or a.situacao=3) order by atividadeTurma.atividade.grupo.abreviacao asc");
			return query.getResultList();
	...
	public List certificadosAlunosComAtividade(Long idAluno, int sit, Long idAtividadeTurma) {
		Query query = null;
			...
			query = manager.createQuery("from Certificado a where a.status is true and a.alunoTurma = " + idAluno
					+ " and a.situacao = " + sit + " and atividadeTurma = " + idAtividadeTurma+" order by atividadeTurma.atividade.grupo.abreviacao asc");
	...
	public List gruposTurmaAluno(Long idTurma) {
		Query query = null;
			...
			query = manager.createQuery("from GrupoTurma a where a.status is true and a.matriz = " + idTurma);
